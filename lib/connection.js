...
var Connection = function(config, stream) {
  EventEmitter.call(this);
  this.config = config || {};
  this.stream = stream || new net.Stream();
...
p.connect = function(cancelConnect) {
  ...
  var onConnectHandler = function onConnectHandler() {
    self.emit('connect');
    //do not start sending data for a cancel connection
    if(!cancelConnect) {
      if(self.config.ssl) {
        self.requestSsl();
      } else {
  ...
  if (this.stream.readyState === 'closed') {
    if(this.config.isDomainSocket) {
      this.stream.connect(this.config.getDomainSocketName());
    } else {
  ...
  this.on('authenticationMD5Password', function(msg) {
    var inner = md5(self.config.password + self.config.user);
