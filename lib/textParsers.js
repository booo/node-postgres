...
var INTERVAL = [YEAR,MON,DAY,TIME].map(
  function(p){
    return "("+p+")?";
  }).join('\\s*');
...
var parseHstore = function parseHstore(val) {

  //postgres escapes the \ and " character
  ...
  while(i < val.length) {
    c = val.charAt(i);
    if(c == '\\') {
      //remove the postgres escampe character from the string
      val = val.substring(start, i) + val.substring(i+1, val.length);
      i = (i - start) + 1;
      start = 0; //reset start because we have a new substring
    } else if(c == '"') {
      //key or value end
...
var init = function(register) {
    ...
    register(1700, function(val){
      if(val.length > maxLen) {
        console.warn('WARNING: value %s is longer than max supported numeric' +
            ' value in javascript. Possible data loss', val);
      }
