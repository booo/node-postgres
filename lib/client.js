...
var Client = function(config, Connection) {
  ...
  this.connection = config.connection || new Connection(this.connectionParameters, stream);
...
p.connect = function(callback) {
  ...
  if(this.connectionParameters.isDomainSocket) {
    con.connect(this.connectionParameters.getDomainSocketName());
  } else {
  ...
  con.on('connect', function() {
    if (self.connectionParameters.ssl) {
      ...
      con.startup({
        ...
        database: self.connectionParameters.database
      });
    }
  });
  ...
  con.on('authenticationCleartextPassword', function() {
    con.password(self.connectionParameters.password);
  });

  //password request handling
  con.on('authenticationMD5Password', function(msg) {
    var inner = md5(self.connectionParameters.password + self.connectionParameters.user);
